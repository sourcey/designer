<template lang="pug">
#site-preview(:class='{"disable-mouse": disableMouse, "expand": expand}')
...
<script>
import Spinner from 'designer/app/javascript/designer/components/Spinner'

import CreateElementDialog from '../views/designer/CreateElementDialog'
import EditElementDialog from '../views/designer/EditElementDialog'
import EditSectionDialog from '../views/designer/EditSectionDialog'
import SectionManagerDialog from '../views/designer/SectionManagerDialog'
import SelectTemplateDialog from '../views/designer/SelectTemplateDialog'
import SocialLinksDialog from '../views/designer/SocialLinksDialog'
import MenuEditDialog from '../views/designer/MenuEditDialog'

import DesignerPlugin from '../plugins/designer'
import { clone } from 'designer/app/javascript/designer/utils'
import { saveSite } from 'dashboard/api'
import axios from 'axios'
import Vue from 'vue'

Vue.use(DesignerPlugin)
...
export default {
  ...
  methods: {
    ...
    previewLoaded () {
      ...
      this.previewState.designerStore = this.designerStore
      this.previewState.designerSpec = this.designerBackendState.spec
      this.designerBackendState.previewUrl = `//${this.$store.state.site.subdomain}.${process.env.SITE_HOST}`
    ...
    setPageUnsavedIfCurrent (reference, flag = true) {
      if (this.isEditingPage(reference)) {
        ...
        console.log('!!!!!!!!!!!!! setPageUnsavedIfCurrent', this.$parent.$refs)
    ...
    createElementData (name) {
      return this.designerStore.createElementData(name)
    ...
    createPage (event) {
      this.selectTemplate(event, (template) => {
        const page = this.designerStore.createDefaultPage(template)
    ...
    insertPageSection (event, page, index) {
      ...
      return new Promise((resolve, reject) => {
        this.selectTemplate(event, (template) => {
          // this.currentPageReference = page.reference // change to page in sidebar
          // navigateToPage(this.item.path)
          // this.editingPageReference = page // change to page in sidebar
          this.setPageUnsavedIfCurrent(page.reference, true)
          this.setCurrentSidebarPage()

          const section = this.designerStore.createSection(page, template, index)
          resolve(section)
        })
      })
    },
    removePageSection (page, sectionId) {
      if (confirm("Are you sure?")) {
        ...
        this.setPageUnsavedIfCurrent(page.reference, true)
        this.setCurrentSidebarPage()
        // this.forcePageUpdate(page.reference)
        return true
      }
    }
  }
