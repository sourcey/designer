    ...
    .item-wrap
      .form-group.designer-select(:title='state.errorText')
        ...
        label.dropdown-label(:for='inputId' v-b-tooltip :title='spec.hint' @click.prevent='showFamilyDialog')
          ...
          icon(name='caret-down' size='16')
    designer-select-input(v-if='value && value.weights' :item='value' name='weight' :spec='fontWeightSpec' @update='emitUpdate')
    designer-select-input(v-if='value' :item='value' name='style' :spec='fontStyleSpec' @update='emitUpdate')
