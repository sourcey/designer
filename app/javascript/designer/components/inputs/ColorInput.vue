...
.item-wrap.color-input
  ...
  .form-group.form-row.align-items-center
    label.form-label(:for="'input-' + name") {{ itemLabel(name, spec) }}
    .col
    ...
    .col
      ...
      verte(ref='verte' model='hex', display='picker', picker='square', menuPosition='right', :value='object', :enableAlpha='false', @input='onPickerChange')
        // :recentColors="null"
        svg
          rect(width='100%', height='100%')
    //
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      this.unwatch = this.$refs.verte.$watch('isMenuActive', (val) => {
