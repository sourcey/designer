...
.item-wrap.trix-text-input
  ...
  //
    ...
    <br>{{object[name]}}
  .form-group
    label(v-if='spec.label !== false', :for='inputName') {{ itemLabel(name, spec) }}
    .editor
...
import { randomString, toggleFullscreen } from '../../utils'
...
export default {
  ...
  data() {
    return {
      ...
      //   this.item[this.name] :
      //   this.$set(this.item, this.name, null) // make it reactive
      //   // this.spec.default
    }
  },
  computed: {
